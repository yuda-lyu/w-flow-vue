...
- **E2E-032** 建線拖曳中之落點可行性即時回饋(單一 case 之承接式 journey,多階段截圖):出發把手標記 origin;其他 source 把手(永不可為落點)一律淡化並顯示 `not-allowed` 游標;hover 不可連落點(他節點之 source 把手、自己節點之任一把手=自我連線)→ 該把手標 `invalid`(danger 紅 ring),預覽線轉 danger 色;hover 合法 target 把手 → 標 `valid`(主題藍 ring + `crosshair`),預覽線轉主題藍實線;於不可連落點放開 → 不建線,且手勢暫態(`data-connect-*` 標記與根 `vue-flow--connecting` class)全數清除。(修正前:所有把手 hover 一律同樣的藍色放大效果,不可連落點與可連者無從分辨,放開無事發生形同故障;對齊 React Flow/Vue Flow 之 connecting/valid 慣例)
...
合計 45 張:31 個 case 各 1 張,E2E-002 佔 12 張,E2E-032 佔 2 張(同 case 兩階段:invalid hover / valid hover)。
...
| E2E-032 | 出發把手 `data-connect-role="origin"`;他 source 把手 computed `opacity < 0.5`;hover 不可連把手 `data-connect-status="invalid"` 且 computed `cursor: not-allowed`,預覽線 class 含 `--invalid`;hover 自己節點把手為 `invalid`;hover 合法把手為 `valid` 且 `cursor: crosshair`,預覽線 class 含 `--valid`;於不可連落點放開後 `conns` 長度不變、`data-connect-*` 標記數為 0、根元素無 `vue-flow--connecting` |
