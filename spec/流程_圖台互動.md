...
- **E2E-044** 觸控平移畫布(契約 §2 pointer 通道):於觸控裝置自畫布空白處以手指拖曳 (−140, −110) → viewport 位移**等於手指位移**;整段拖曳期間**滑鼠事件 0 次**(瀏覽器只在點按結束後補送相容滑鼠事件,拖曳期間一個都不送,故此路徑只能由 pointer 通道承擔);`.vue-flow` 之 computed `touch-action` 為 `none`(未宣告時瀏覽器會在手指移動數 px 後接手頁面捲動並以 pointercancel 中斷事件流,圖台只收得到 2~3 個 pointermove)。
- **E2E-045** 觸控拖曳節點:自節點本體以手指拖曳 (+90, +70) → 該節點座標位移等於手指位移 ÷ zoom、發出 `update:nodes`,其餘節點不動。
- **E2E-046** 觸控四角縮放:先以 tap 選取節點使四角出現(觸控無 hover,tap 之相容事件補 `mouseover`/`mouseenter` 且選取本身亦使四角顯示)→ 以手指拖右下角 (+60, +40) → 寬高各增該量 ÷ zoom、座標不變、發出 `update:nodes`。
- **E2E-047** 觸控建線:自節點 2 之右把手以手指拖至節點 3 之左把手 → `conns` 增 1 且新邊之 from/to 為該兩節點、發出 `update:conns`。
- **E2E-048** 觸控拖曳轉折點:前置於畫布空白處設定 `e1-2` 之 `points`(setup,非 act)→ 以手指拖該轉折點 (+60, +45) → `points` 更新等量、發出 `conn-settings-update`。
- **E2E-049** 觸控雙指捏合縮放(契約 §1 捏合列):兩指自相距 120px 張開至 216px(距離比 1.8)→ zoom 變為 1.8 倍(受 `zoomMin`/`zoomMax` 夾)、**兩指中點處之內容座標不變**(錨點語義與滾輪縮放同一 `zoomAroundPoint`);`opt.zoomOnPinch: false` 時同一手勢不改變 zoom。
- **E2E-050** 觸控點按不啟動拖曳類手勢(契約 §2 門檻延後):tap 節點 → 選取且開資訊 popup(點按語義由相容滑鼠事件承擔,與滑鼠一致);tap 畫布空白 → 清除選取;tap 把手 / 四角 / 轉折點 → **不建線、不改尺寸、不移動轉折點**(接觸當下不啟動,跨拖曳門檻才啟動),全程 `conns` 長度、節點寬高與 `points` 皆不變。
...
| E2E-044 | `flow-E2E-044-touch-panned.png` | 1 | 已產製 |
| E2E-045 | `flow-E2E-045-touch-node-dragged.png` | 1 | 已產製 |
| E2E-046 | `flow-E2E-046-touch-node-resized.png` | 1 | 已產製 |
| E2E-047 | `flow-E2E-047-touch-conn-created.png` | 1 | 已產製 |
| E2E-048 | `flow-E2E-048-touch-waypoint-dragged.png` | 1 | 已產製 |
| E2E-049 | `flow-E2E-049-touch-pinch-zoom.png` | 1 | 已產製 |
| E2E-050 | `flow-E2E-050-touch-tap-no-gesture.png` | 1 | 已產製 |

合計 69 張:44 個 case 各 1 張,E2E-002 佔 12 張,E2E-032 / E2E-034(invalid hover / valid hover)、E2E-036(初始 / 編輯後)與 E2E-037(節點 / 連線)各佔 2 張,E2E-041 佔 5 張。
...
| E2E-044 | 觸控手勢以 CDP `Input.dispatchTouchEvent` 送出(走瀏覽器真實輸入管線,非 JS 合成事件;Playwright 無觸控拖曳 API,此為該層級之唯一入口);拖曳前後 `vm.viewport` 之 x/y 差等於手指位移(誤差 <1px)、zoom 不變;整段手勢期間頁面記錄之 `mousedown`/`mousemove`/`mouseup` 次數皆為 0;`.vue-flow` computed `touchAction === 'none'` |
| E2E-045 | 節點 `position` 位移 === 手指位移 ÷ zoom(誤差 <1px);已發出 `update:nodes`;未被拖曳之節點座標不變 |
| E2E-046 | tap 後 `.vue-flow__resize` 元素數為 4(觸控下 affordance 可達);拖曳後 `width`/`height` 各增 手指位移 ÷ zoom(誤差 <1px)、`position` 不變;已發出 `update:nodes` |
| E2E-047 | `conns.length` 增 1;新邊之 `from === '2'`、`to === '3'`;已發出 `update:conns` |
| E2E-048 | `conn.points[0]` 位移 === 手指位移 ÷ zoom(誤差 <1px);已發出 `conn-settings-update` |
| E2E-049 | 捏合後 `viewport.zoom` ÷ 捏合前 zoom ≈ 1.8(誤差 <0.02);兩指中點處之內容座標(`screenToFlow` 換算)捏合前後差 <1px;`opt.zoomOnPinch: false` 時同一手勢後 zoom 不變 |
| E2E-050 | tap 節點後 `selectedNodes === ['2']` 且資訊 popup 可見;tap 空白後 `selectedNodes === []`;tap 把手後 `conns.length` 不變且 `isConnecting === false`;tap 四角後節點 `width`/`height` 不變;tap 轉折點後 `points` 不變 |
