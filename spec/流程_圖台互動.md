...
- **E2E-027** 未上鎖時自某節點連接點拖曳至另一節點連接點 → 建立新連線,發出 `update:conns`;新 conn 只有 `id`/`from`/`to`——**邊沒有自己的方位**,兩端方向由節點之 To/From Handle 決定(`流程_互動契約.md` §4)。
...
- **E2E-031** 於節點設定表單將 To Handle 改為 Right → 該節點之 source 把手移至右側,其全部出邊之出發端一併跟隨改道;邊資料不含方位欄位。
...
| E2E-027 | `conns` 新增一筆,其 `from` / `to` 為該兩節點;`update:conns` 已發出;新 conn 不含 `fromPosition`/`toPosition` |
...
| E2E-031 | source 把手之 `data-handle-position` 為 `right`;出邊之路徑 d 改變;conn 資料不含 `fromPosition` |
