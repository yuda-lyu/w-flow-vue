...
describe('O1 每鍵三態', () => {
    test('未給 → 預設', () => {
        ...
        expect(r.center).toEqual([0, 0]); expect(r.panLimits).toBeNull(); expect(r.fitViewOnInit).toBe(true); expect(r.fitViewPadding).toBe(50)
    ...
    test('明確值採用; 依 kind 回退', () => {
        ...
        expect(resolveOptValue({ fitViewPadding: 0 }, 'fitViewPadding')).toBe(0)
        expect(resolveOptValue({ fitViewPadding: -1 }, 'fitViewPadding')).toBe(50)
        expect(resolveOptValue({ fitViewPadding: '10' }, 'fitViewPadding')).toBe(50)
